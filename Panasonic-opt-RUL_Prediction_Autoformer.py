...
# Autoformer超参数搜索空间定义
# 只包含AutoFormerNetModel中定义的参数
HYPERPARAMETER_SPACE = {
    'seq_len': [32, 64, 128],                     # 输入序列长度（对应iTransformer的input_size）
    'label_len': [0, 16, 32],                     # 标签长度
    'pred_len': [1, 3, 5],                        # 预测长度（对应iTransformer的output_size）
...
class AutoformerOptimizer:
    """Autoformer RUL预测优化器，用于系统化的超参数调优和重复实验"""
        ...
        for i, param_combo in enumerate(param_combinations):
            ...
            try:
                ...
                if results is not None:
                    ...
                    # 保存单次预测结果为CSV
                    param_str = f"sl{hyperparams['seq_len']}_pl{hyperparams['pred_len']}_dm{hyperparams['d_model']}_el{hyperparams['e_layers']}_dl{hyperparams['d_layers']}"
...
def main():
    ...
    parser.add_argument('--model', default='Autoformer5', help='Model name.')
    ...
    # 创建优化器
    optimizer = AutoformerOptimizer(args)
    
    # 定义自定义参数搜索空间 - 针对Autoformer的特定参数（去掉n_heads）
    custom_param_grid = {
        'seq_len': [64],                          # 输入序列长度
        'label_len': [0, 16],                     # 标签长度
        'pred_len': [1],                          # 预测长度
