...
class RULMambaOptimizer:
    """RULMamba RUL预测优化器，用于系统化的超参数调优和重复实验"""
    ...
    def prepare_data(self, battery_data, start_point, hyperparams):
        ...
        max_prediction_length = hyperparams['predict']
        max_encoder_length = hyperparams['lookback']
    ...
    def train_single_model(self, training, validing, testing, df_all, start_point, hyperparams, run_seed=None):
        ...
        model = RULMambaNetModel.from_dataset(
            ...
            predict=hyperparams['predict'],
            enc_in=len(training.time_varying_known_reals),
            dec_in=4,
            d_model=hyperparams['d_model'],
            d_ff=hyperparams['d_ff'],
            n_enc_layer=hyperparams['n_enc_layer'],
            n_dec_layer=hyperparams['n_dec_layer'],
        ...
        for i, param_combo in enumerate(param_combinations):
            ...
            try:
                ...
                if results is not None:
                    ...
                    # 保存单次预测结果为CSV
                    param_str = f"lb{hyperparams['lookback']}_pred{hyperparams['predict']}_dm{hyperparams['d_model']}"
...
def main():
    ...
    # 加载电池数据
    battery_data = np.load('.\Panasonic\Panasonic_Data.npy', 
                          allow_pickle=True).item()
    
    # 创建优化器
    optimizer = RULMambaOptimizer(args)
